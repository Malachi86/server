...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      if (!success) {
        // The AuthContext now shows an alert on failure
      }
      // On success, the App will redirect automatically
    } else {
      // --- Registration Logic ---
      ...
      // Simple teacher code validation (can be moved to backend later)
      if (formData.role === 'teacher' && (document.getElementById('teacherCode') as HTMLInputElement).value !== 'AMACC2025') {
            ...
            <div className="p-12">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-[#b40000] hover:bg-[#8b0000] text-white py-3 rounded-lg transition-colors duration-200 flex items-center justify-center gap-2 disabled:bg-gray-400"
                >
                  {isLoading ? (
                    'Processing...'
                  ) : isLogin ? (
                ...
                <div className="text-center mt-6">
                  <button
                    ...
                    className="text-[#b40000] hover:underline disabled:text-gray-500"
                    disabled={isLoading}
                  >
