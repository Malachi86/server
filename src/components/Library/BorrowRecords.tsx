...
// Define an interface for the borrow record object for type safety
interface IBorrowRecord {
  ...
  borrowed_at: string;
  due_date: string;
  returned_at?: string; // This can be optional
}
...
export default function BorrowRecords() {
  ...
  const loadRecords = () => {
    const allRecords: IBorrowRecord[] = JSON.parse(localStorage.getItem('borrow_records') || '[]');
    // Sort with explicit types to avoid implicit 'any' error
