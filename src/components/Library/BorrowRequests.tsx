...
export default function BorrowRequests() {
  ...
  const loadRequests = () => {
    const allRequests: IBorrowRequest[] = JSON.parse(localStorage.getItem('borrow_requests') || '[]');
  ...
  const handleApprove = (requestId: string) => {
    ...
    if (request) {
      ...
      const book = books.find((b) => b.barcode === request.book_barcode);
      ...
      if (book) {
        ...
        // Add to records
        const records: IBorrowRecord[] = JSON.parse(localStorage.getItem('borrow_records') || '[]');
        records.push({
          id: `REC${Date.now()}`,
  ...
  const markReturned = () => {
    ...
    const record = records.reverse().find(
      (r) => r.book_barcode === barcode && r.student === student && !r.returned_at
    );
    if (record) {
