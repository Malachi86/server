...
// Define an interface for the enrollment object for type safety
...
export default function EnrolledStudents() {
  const { user } = useAuth();
  const [students, setStudents] = useState<IEnrollment[]>([]);
  ...
  const loadStudents = () => {
    ...
    const approved = enrollments.filter(
      (e) => e.teacher === user?.usn && e.status === 'Approved'
    );

    // Explicitly cast to string[]
    const uniqueSubjects: string[] = Array.from(new Set(approved.map((e) => e.subject)));
