...
interface ISession {
  ...
  start_iso: string;
  end_iso?: string;
  ended_iso?: string; // Legacy or alternative property
  active: boolean;
}

interface IPCHistoryEntry {
  student_name: string;
  student: string;
  start: string;
  end: string;
...
interface IPCHistory {
  [key: string]: IPCHistoryEntry[];
}

export default function MySessions() {
  const { user } = useAuth();
  const [sessions, setSessions] = useState<ISession[]>([]);
  const [pcHistory, setPCHistory] = useState<IPCHistory>({});
  ...
  const loadSessions = () => {
    ...
    const pcHistoryData: IPCHistory = JSON.parse(localStorage.getItem('pc_history') || '{}');
    
    const mySessions: ISession[] = attendance[user.usn] || [];
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 flex items-center gap-2">
                    <Clock size={14} />
                    {new Date(session.start_iso).toLocaleString()} - {new Date(session.ended_iso || session.end_iso || new Date()).toLocaleString()}
                  </p>
                </div>
