...
export default function MakeRequest() {
  ...
  const loadTeachers = () => {
    ...
    // Start with the Administrator option to ensure it's always first and unique
    const uniqueTeachers: any[] = [{ usn: 'admin', name: 'Administrator' }];
    const seen = new Set(['admin']);

    const myApproved = enrollments.filter(
      (e: any) => e.student === user?.usn && e.status.toLowerCase() === 'approved'
    );

    myApproved.forEach((e: any) => {
      // If teacher is not 'admin' and not already seen, add them to the list
    ...
    console.log('Final teachers list:', uniqueTeachers); // For debugging
    setTeachers(uniqueTeachers);
  };

  const loadSubjects = (teacherUsn: string) => {
    if (teacherUsn === 'admin') {
      setSubjects(['Exam', 'Personal Use']);
      return;
    }
    ...
    const mySubjects = enrollments
      .filter((e: any) => e.student === user?.usn && e.teacher === teacherUsn && e.status.toLowerCase() === 'approved')
      .map((e: any) => e.subject);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newRequest = {
      ...
      teacher_name: formData.teacher === 'admin' ? 'Administrator' : users[formData.teacher]?.name,
