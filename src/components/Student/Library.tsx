...
export default function Library() {
  ...
  const [bookInfo, setBookInfo] = useState<ILibraryBook | null>(null);
  const [myRequests, setMyRequests] = useState<IBorrowRequest[]>([]);
  ...
  const loadMyRequests = () => {
    if (!user) return;
    const requests: IBorrowRequest[] = JSON.parse(localStorage.getItem('borrow_requests') || '[]');
    const mine = requests.filter((r) => r.student === user.usn);
  ...
  const handleLookup = () => {
    ...
    const books: ILibraryBook[] = JSON.parse(localStorage.getItem('library_books') || '[]');
    const book = books.find((b) => b.barcode === barcode.trim());
    ...
    if (book.status === 'Available') {
      if (window.confirm(`Request to borrow "${book.title}"?`)) {
        ...
        const newRequest: IBorrowRequest = {
          id: `BORR${Date.now()}`,
          student: user!.usn,
