...
ENROLLMENTS_FILE = "backend/enrollments.json"
AUDIT_FILE = "backend/audit.json"
SUBJECTS_FILE = "backend/subjects.json"
TEACHERS_FILE = "backend/teachers.json"
REQUESTS_FILE = "backend/requests.json"
ATT_FILE = "backend/attendance.json"
SETTINGS_FILE = "backend/settings.json"
LIBRARY_FILE = "backend/library.json"
BORROW_FILE = "backend/borrow_requests.json"
BORROW_RECORDS_FILE = "backend/borrow_records.json"
...
if __name__ == "__main__":
    ...
    all_files = [
        ...
        (REQUESTS_FILE, []),
        (ATT_FILE, []),
        (SETTINGS_FILE, {}), # Settings might be a dict
        (LIBRARY_FILE, []),
        (BORROW_FILE, []),
        (BORROW_RECORDS_FILE, [])
    ]
    ...
    with socketserver.TCPServer(("", PORT), SimpleHTTPRequestHandler) as httpd:
        print(f"Serving at port {PORT}. All data files initialized.")
        httpd.serve_forever()
