...
export default function TeacherDashboard() {
  ...
  const showSubjectStudents = (subject: string) => {
    ...
    const approved = enrollments.filter(
      (e: any) => e.teacher === user?.usn && e.subject === subject && e.status === 'Approved'
    );

    let message = `Subject: ${subject}\n\nEnrolled Students:\n\n`;
    
    approved.forEach((enrollment: any, idx: number) => {
      const studentSessions = attendance[enrollment.student] || [];
      const subjectSessions = studentSessions.filter(
        (s: any) => s.teacher === user?.usn && s.subject === subject
      );
      ...
      if (subjectSessions.length > 0) {
        ...
        message += `   Last session: ${new Date(lastSession.start_iso).toLocaleDateString()}\n`;
      }
      message += '\n';
    });

    alert(message || 'No enrolled students yet');
  };
