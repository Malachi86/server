...
export default function RoomReservations() {
  ...
  const loadData = () => {
    ...
    const myReservations = allReservations.filter((r: any) => r.teacher === user?.usn);
  ...
  const addReservation = () => {
    ...
    const schedule = subject.schedules?.find((s: any) => s.day === dayName.substring(0, 3));
  ...
  const deleteReservation = (id: string) => {
    ...
    const allReservations = JSON.parse(localStorage.getItem('reservations') || '[]');
    const updated = allReservations.filter((r: any) => r.id !== id);
